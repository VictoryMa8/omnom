...
const parseMode = ref<'ai' | 'exact'>('ai');
...
const handlePromptSubmit = async (prompt: string, mode: 'ai' | 'exact' = 'ai') => {
  ...
  try {
    activeParsedResult.value = await AiApi.parseMeal(prompt, undefined, parseController.signal, mode);
  } catch (err: any) {
...
<template>
  ...
  <header class="site-header">
    <div class="nav-inner">
      <a href="#" class="brand" aria-label="omnom AI home"><img class="brand-mark" src="/favicon.svg" alt="" />omnom <span class="ai-tag">AI</span></a>
  ...
  <main class="app-main">
    ...
    <div class="dashboard-layout">
      <aside class="summary-column"><DailySummaryHeader />
        <div class="quiet-note"><span class="tiny-dot"></span><p>Saved on this device. <button class="inline-link" @click="showSettingsModal = true">Back up in settings <ArrowUpRight class="inline-icon" /></button></p></div>
      </aside>
      <div class="diary-column">
        <QuickLogInput :loading="isParsing || isSavingMeal" @submit="handlePromptSubmit" />
        <div v-if="isParsing" class="status-note" role="status"><Loader2 class="icon animate-spin" /><span>{{ parseMode === 'exact' ? 'Reading your meal…' : 'Putting your meal together… up to 35 seconds.' }}</span><button class="text-button" @click="cancelParsing">Cancel</button></div>
    ...
    <footer class="site-footer"><span>omnom AI</span></footer>
  </main>
